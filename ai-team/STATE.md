...
| | |
|---|---|
| 최종 갱신 | **2026-08-29 · 초기화 + 개편 v3 + 회의실 전원 점호(D-027)** |
| 갱신 주체 | 원격 컨테이너 |
...
| B-07 | **감사 미수행 9건** — D-021 · D-025 ~ D-032 | 🔴 전부 `.claude/` 를 고쳤고 D-014 요건 ③ 이 비어 있습니다. 감사실 2인 호출 필요 |
...
"회의실" 이라고 하면 **31명 전원 점호**부터 합니다. 발언하지 않는 사람도 이름과 **대기 사유**가 출석부에 나옵니다.
```bash
...
📱💻 **팀은 이미 기기 간 이동 중** — 핸드폰 claude.ai 앱도 같은 컨테이너·같은 저장소·같은 31명 (D-031)
🎩 **음성 브리핑 콘솔** (브라우저, 설치 불필요) — 원본 `ai-team/console/jarvis-console.html`
🎩 **자비스 상주 프로그램 `jarvis.py`** — 박수 두 번 → "네, 사장님" → 보고문 낭독 → 받아쓰기. 설치 가이드 `ai-team/JARVIS-설치.md` (로직 8종 검증 · **마이크·소리·단축키는 사장님 PC 에서 검증 필요**) · 🔊 응답 루프 `jarvis-voice.ps1` (미검증) · 절차 `ai-team/voice.md`
🎙️ 종전: **말로 명령은 `tap` 모드로 가능** · **박수 두 번·상시 마이크는 기능 부재로 불가** (`ai-team/voice.md`)
🔊 **음성 보고도 원격에서 불가** (TTS·오디오 장치 실측 부재). `speak-report.ps1` 로 **Windows 로컬에서** 읽습니다 — 미검증.
