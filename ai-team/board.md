# 작업 현황판

> 실시간 현황입니다. 세션·사이클이 진행되며 계속 갱신합니다.

## 지금
| | |
|---|---|
| 상태 | **사이클 착수 불가** — 감사 반려가 유효합니다 (B-07) |
| 마지막 작업 | 2026-08-30 감사 1·2차 + 재감사 → 반려 시정 (D-036 · D-037) |
| 다음 | **재감사 3건**(1차 계열 · 2차 · 3차) → 전부 통과해야 사이클 3. 치명 전건 시정 완료 |

## 🔴 차단 — 사용자 지시가 있어야 풀립니다
| # | 항목 | 상태 |
|---|---|---|
| B-01 | `.claude/` 내부 모순 (`team-org.md` 자동 실행 서술) | ✅ **2026-08-29 해소** — 정확한 서술로 교체 |
| B-02 | 에이전트 산출 경로에 앱 폴더 접두사 없음 | ✅ **해소** — 전 에이전트 `<앱폴더>/` 적용, 규칙을 `team-org.md` 에 명문화 |
| B-03 | Routine 프롬프트 교체 | 🔴 **미해소** — 이 환경에서 Routine 을 수정할 수 없습니다. `ai-team/routine-prompt.md` 의 교체안을 사용자가 claude.ai Routines UI 에 반영해야 합니다 |
| B-04 | `handoff/03·04` 필독 명시 없음 | ✅ **해소** — `tech-lead`·`qa-lead` 착수 전 필독 신설, `gtm-lead` 강화 |
| B-05 | `daily-app` 4-1 ↔ 5 순서 결함 | ✅ **해소** — STATE 갱신을 노션 정리 **뒤**(6단계)로 이동 |
| ~~B-06~~ | ~~브랜치 3갈래 분기~~ (`8tlqr8` · `69oi8o` · `knz8ob`) | 🔴 **미해소** — 실제 작업과 자동 푸시는 `claude/meeting-room-team-structure-knz8ob`. `8tlqr8` 은 20명 조직이며 Routine 이 그쪽을 봅니다. 사용자 결정 필요 |
| B-07 | **D-025 개편 감사 미수행** | 🔴 신규 — 감사실 2인 호출 필요 (D-035 포함 **12건**) |
| B-08 | **슈퍼베이스 키 미설정** | 🔴 메커니즘(D-035)은 완성·가짜서버 실측했으나 이 컨테이너에 `SUPABASE_URL`·`SUPABASE_KEY` 가 없어 **새 경로(`push --team`·`restore`·`verify`·부트스트랩)는 실제 서버를 거친 적이 없습니다.** (D-023 의 상태 8파일 왕복은 실재 — 3차 감사 중대-7 로 범위 정정) 키를 넣고 `push --team` 1회를 돌려야 다른 창에서 나옵니다 |

## 기기 분담
| 기기 | 역할 | 자동 실행 |
|---|---|---|
| 원격 컨테이너 (claude.ai) | **실행.** 사이클 진행 후 커밋·푸시 | ✅ 여기서만 |
| Windows 로컬 | 열람·이어서 작업 | ⚠️ 돌면 안 됨 — 중지 여부 미확인 |
| 노션 「📓 작업 일지」 | 사람이 읽는 기록 | 항목 append + 상단 표 갱신 |
| 슈퍼베이스 | 팀이 읽는 상태 | 마감 시 push |
